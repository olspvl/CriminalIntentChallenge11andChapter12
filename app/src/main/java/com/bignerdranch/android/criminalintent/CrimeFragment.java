...
public class CrimeFragment extends Fragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mPosition = (int) getArguments().getSerializable(ARG_CRIME_POSITION);
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mToFirstCrimeButton = v.findViewById(R.id.first_crime_button);
        mToLastCrimeButton = v.findViewById(R.id.last_crime_button);

        if(mPosition == 0)
            mToFirstCrimeButton.setEnabled(false);
        if(mPosition == mCrimeLab.getSize() - 1)
            mToLastCrimeButton.setEnabled(false);

        mToFirstCrimeButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = CrimePagerActivity.newIntent(getActivity(), mCrimeLab.getUUID(0));
        ...
        mToLastCrimeButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = CrimePagerActivity.newIntent(getActivity(), mCrimeLab.getUUID(mCrimeLab.getSize() - 1));
                startActivity(intent);
                getActivity().finish();
            }
        });
